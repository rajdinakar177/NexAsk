...
export async function POST(request: NextRequest) {
    try {
        ...
        // get question or answer to find author
        ...
        // CHANGE VOTE
            ...
            await databases.updateDocument(db, voteCollection, existing.$id, { voteStatus });
        ...
        await users.updatePrefs(authorId, {
            reputation: Math.max(0, Number(prefs.reputation || 0) + reputationChange),
        });
        ...
        return NextResponse.json(
            { error: error?.message || "Error while creating vote" },
